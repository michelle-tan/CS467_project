...
<template>
  <b-form ref="form" @submit.prevent="handleSubmit">
    <b-alert
      ...
      dismissible
      :show="showFailure"
    >Something's gone awry, please try again later!</b-alert>

    <div v-for="(input, key) in formData" :key="key">
      <b-form-group :label="key|wordify|capitalize|colonize">
        <b-form-input :type="key|inputType" v-model="formData[key]" required></b-form-input>
      </b-form-group>
    </div>

    <b-form-group v-if="formData.password" label="Re-type password:">
      <b-form-input
        type="password"
        required
...
export default {
  ...
  computed: {
    ...
    validateForm() {
      if (this.isMounted) {
        return this.validatePassword.isValid && this.$refs.form.checkValidity();
      } else {
        return false;
      }
    }
  ...
  methods: {
    handleSubmit() {
      ...
      submitObject.append(
        "formData",
        JSON.stringify({ ...this.formData, isSeller: this.isSeller })
      );
